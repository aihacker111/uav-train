...
from lib.tracker.multitracker import MCJDETracker, HawkDetMCTracker
...
def eval_seq(opt,
    ...
    is_hawkdet = 'hawkdet' in opt.arch
    tracker = HawkDetMCTracker(opt, frame_rate) if is_hawkdet else MCJDETracker(opt, frame_rate)
...
def main(opt,
    ...
    # ECViT backbone always needs ImageNet normalization regardless of how opt was created
    use_imagenet_norm = getattr(opt, 'use_imagenet_norm',
                                'hawkdet' in opt.arch or 'hybrid' in opt.arch
                                or 'ecdet_jde' in opt.arch)
