...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        stacked_hs = torch.stack(dec_out_hs)   # (num_layers, B, K, hidden_dim)
        ...
        if spatial_features is not None:
            ...
            return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
                ...
                pre_bboxes, pre_scores, dec_out_segs[-1], stacked_hs
        else:
            return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
                torch.stack(dec_out_pred_corners), torch.stack(dec_out_refs), None, \
                pre_bboxes, pre_scores, None, stacked_hs
    ...
    def _generate_anchors(self,
        ...
        if spatial_shapes is None:
            ...
            for s in self.feat_strides:
                spatial_shapes.append([math.ceil(eval_h / s), math.ceil(eval_w / s)])
    ...
    def forward(self, feats, targets=None, spatial_feat=None, heatmap_ref_points=None):
        ...
        if self.training:
            out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], 'pred_corners': out_corners[-1],
                    ...
                    'hs': dec_out_hs[-1]}   # (B, K, hidden_dim) — final decoder layer hidden states
        else:
