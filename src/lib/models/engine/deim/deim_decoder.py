...
class DEIMTransformer(nn.Module):
    ...
    def __init__(self,
      ...
      # denoising
        ...
        # Optional per-query ReID head for JDE-style tracking.
        # reid_dim=0 (default) disables it — pure detection mode.
        ...
        if reid_dim > 0:
            ...
            nn.init.xavier_uniform_(self.reid_head[0].weight)
            nn.init.xavier_uniform_(self.reid_head[2].weight)
            nn.init.zeros_(self.reid_head[0].bias)
            nn.init.zeros_(self.reid_head[2].bias)
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits, hs_last = self.decoder(
            init_ref_contents,
        ...
        # ReID head: per-query L2-normalised embedding for JDE-style tracking
        if self.reid_dim > 0:
            out['pred_reid'] = F.normalize(self.reid_head(hs_last), dim=-1)
