...
def _load_ecdet_pretrained(model: ECDetJDE, ckpt_path: str):
    """
    Load ECDet COCO checkpoint into ECDetJDE, skipping keys that are
    intentionally incompatible:

      decoder.anchors / decoder.valid_mask
          Resolution-dependent buffers (640×640 in COCO checkpoint vs
          current eval_spatial_size). Regenerated by the model at runtime.

      decoder.denoising_class_embed
      decoder.enc_score_head
      decoder.dec_score_head.*
          Classification heads trained on 80 COCO classes; current model
          has a different num_classes. These are fine-tuned from scratch.

      decoder.reid_head.*
          New head not present in the COCO checkpoint.
    """
    ...
    # Keys that must be skipped (shape or semantic mismatch by design)
    _SKIP_PREFIXES = (
        'decoder.anchors',              # resolution buffer — rebuilt at runtime
        'decoder.valid_mask',           # resolution buffer — rebuilt at runtime
        'decoder.denoising_class_embed', # num_classes-dependent
        'decoder.enc_score_head',        # num_classes-dependent
        'decoder.dec_score_head',        # num_classes-dependent
        'decoder.reid_head',             # new head, not in COCO checkpoint
    )

    skipped, loaded_state = [], {}
    ...
    for k, v in state.items():
        if any(k.startswith(p) for p in _SKIP_PREFIXES):
            skipped.append(k)
            continue
        # Guard against any remaining shape mismatch
        if k in model_shapes and model_shapes[k] != v.shape:
            skipped.append(f'{k} [shape {v.shape}→{model_shapes[k]}]')
            continue
        loaded_state[k] = v
    ...
    print(
        f'[ECDetJDE] Pretrained loaded — '
        f'loaded: {len(loaded_state)} | '
        f'skipped (expected): {len(skipped)} | '
        f'missing: {len(missing)} | '
        f'unexpected: {len(unexpected)}'
    )
    truly_missing = [k for k in missing if not any(k.startswith(p) for p in _SKIP_PREFIXES)]
    if truly_missing:
        print(f'[ECDetJDE] WARNING — unexpected missing keys: {truly_missing[:10]}')
