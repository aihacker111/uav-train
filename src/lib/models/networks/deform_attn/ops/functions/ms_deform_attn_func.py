...
# Lazy import: only load the compiled CUDA extension when actually called.
# If MultiScaleDeformableAttention is not compiled, MSDeformAttn will fall back
# to ms_deform_attn_core_pytorch automatically (see ms_deform_attn.py).
_MSDA = None
...
def _get_msda():
    global _MSDA
    if _MSDA is None:
        import MultiScaleDeformableAttention as _mod
        _MSDA = _mod
    return _MSDA
