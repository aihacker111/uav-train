...
class HybridDEIM(nn.Module):
    ...
    # Keys to skip when loading a checkpoint with a different num_classes.
    # The decoder.__init__ already creates these layers with the correct class
    # count and applies proper init (focal-loss prior bias for Linear heads;
    # normal_ for denoising_class_embed). Just skip, nothing else needed.
    _CLASS_HEAD_RE = re.compile(
        r'^decoder\.(denoising_class_embed|enc_score_head|dec_score_head\.\d+)\.'
        r'(weight|bias)$'
    )
    ...
    def load_pretrained(self, path: str) -> None:
        ...
        # Skip class-head keys — shapes always mismatch when fine-tuning with a
        # different num_classes. decoder.__init__ sets these correctly already.
        skipped = [k for k in state if self._CLASS_HEAD_RE.match(k)]
        if skipped:
            print(f'[HybridDEIM] skipping {len(skipped)} class-head tensors '
                  f'(num_classes mismatch): {skipped[:4]}{"…" if len(skipped) > 4 else ""}')
