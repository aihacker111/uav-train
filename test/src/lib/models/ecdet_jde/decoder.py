    ...
    def forward(self,
        ...
        if spatial_features is not None:
            ...
            return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
                ...
                pre_bboxes, pre_scores, dec_out_segs[-1], dec_out_hs[-1]
        else:
            return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
                torch.stack(dec_out_pred_corners), torch.stack(dec_out_refs), None, \
                pre_bboxes, pre_scores, None, dec_out_hs[-1]
...
class ECTransformer(nn.Module):
    ...
    def __init__(self,
      ...
      # denoising
        ...
        # ReID head: maps query hidden state → embedding vector
    ...
    def forward(self, feats, targets=None, spatial_feat=None):
        ...
        # decoder
        out_bboxes, out_logits, out_corners, out_refs, out_masks, pre_bboxes, pre_logits, pre_segs, last_dec_hs = self.decoder(
                spatial_feat,
                init_ref_contents,
        ...
        # ReID: apply reid_head to last-layer detection query hidden states
        # last_dec_hs: (B, N_total, hidden_dim) where N_total = N_dn + N_det during training
        # After DN split above, last_dec_hs already contains full queries;
        # slice to detection queries only (last num_queries)
        det_hs = last_dec_hs[:, -self.num_queries:, :]  # (B, N_det, hidden_dim)
        pred_reid = self.reid_head(det_hs)              # (B, N_det, reid_dim)
